...
function build_xos_docker_images() {
    ...
    ssh ubuntu@xos-1 "cd xos; git config --global user.email 'ubuntu@localhost'; git config --global user.name 'XOS ExampleService'"
    ssh ubuntu@xos-1 "cd xos; git checkout $XOS_BRANCH"

    echo "Changing git port to use HTTPS"
    ssh ubuntu@xos-1 "cd xos; grep -rl \"git://\" . | xargs sed -i 's/git\:\/\//https\:\/\//g'"
    
    echo "Rebuilding XOS containers"
    ssh ubuntu@xos-1 "cd xos/containers/xos; make base"
    ssh ubuntu@xos-1 "cd service-profile/cord-pod; make local_containers"
}

function setup_xos() {

    echo "Setting up XOS, will take a few minutes"
    ssh ubuntu@xos-1 "cd service-profile/cord-pod; make"
    echo ""
    echo "Pause 2 minutes"
    sleep 120

    ssh ubuntu@xos-1 "cd service-profile/cord-pod; make vtn; make fabric"
    echo ""
...
function run_e2e_test() {
    ...
    ssh ubuntu@nova-compute-1 "sudo lxc-attach -n testclient -- ping -c 3 8.8.8.8"
...
XOS_BRANCH="master"
XOS_REPO_URL="https://github.com/girishgc/xos.git"
DIAGNOSTICS=1
